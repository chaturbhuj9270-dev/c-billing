...
class CompanyListWidget extends StatelessWidget {
  ...
  final void Function(Map<String, dynamic>) onCompanyTap;
  final void Function(Map<String, dynamic>)? onCompanyLongPress;
  ...
  const CompanyListWidget({
    ...
    this.onCompanyLongPress,
    this.emptyTitle = 'No companies yet',
    this.emptySubtitle = 'Add your first company to get started',
    this.noResultsTitle = 'No results found',
    this.noResultsSubtitle = 'Try a different search term',
  });

  List<Map<String, dynamic>> get _filteredAndSortedCompanies {
    ...
    filtered.sort((a, b) {
      ...
      switch (sortField) {
        case CompanySortField.name:
          ...
          break;
        case CompanySortField.createdDate:
          final aDate = a['createdAt'] as DateTime? ?? DateTime(2000);
          final bDate = b['createdAt'] as DateTime? ?? DateTime(2000);
          result = aDate.compareTo(bDate);
          break;
        case CompanySortField.companyCode:
          final aCode = (a['companyCode'] ?? '').toString();
          final bCode = (b['companyCode'] ?? '').toString();
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _filteredAndSortedCompanies;
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return RepaintBoundary(
            child: _CompanyCard(
              company: filtered[index],
              onTap: () => onCompanyTap(filtered[index]),
              onLongPress: onCompanyLongPress != null
                  ? () => onCompanyLongPress!(filtered[index])
                  : null,
            ),
          );
        },
...
class _CompanyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final contact = (company['contact'] ?? '').toString();
    final address = (company['address'] ?? '').toString();
    final companyCode = (company['companyCode'] ?? '').toString();
    final isActive = company['isActive'] as bool? ?? true;
    final isSynced = company['isSynced'] as bool? ?? true;
    ...
    final accentColor =
        accentColors[((company['id'] ?? '').hashCode.abs()) % 4];
